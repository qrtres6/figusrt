import { redis, normName, hashPin, signToken, readBody, dbErrorResponse } from '../lib/store.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ token: signToken(key), name: u.name });
  } catch (e) {
