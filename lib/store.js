...
export async function setProgress(key, data) {
  ...
  if (b === 'pg') {
    ...
    // postgres.js serializes objects to JSON automatically for jsonb columns.
    await s`INSERT INTO album_progress (name_key, data, ts)
...
export async function dbStats() {
  ...
  if (b === 'pg') {
    const s = await pg();
    const u = await s`SELECT count(*)::int AS n FROM album_users`;
    const p = await s`SELECT count(*)::int AS n FROM album_progress`;
    out.users = u[0].n; out.progress = p[0].n;
  } else if (b === 'redis') {
    out.note = 'redis conectado (sin conteo)';
  }
  return out;
}
...
export function dbErrorResponse(res, e) {
  ...
  return res.status(500).json({ error: 'Error del servidor.', detail: (e && e.message) || String(e) });
}
